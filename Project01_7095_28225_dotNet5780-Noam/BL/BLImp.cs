...
namespace BL
{
    ...
    public class BLImp : IBL
    {
        ...
        public void UpdateOrder(int key, Enums.OrderStatus stat)
        {
            ...
            {
                //TODO: finish the get Commission here -dont now how to do it

                //Update the Diary
                HostingUnit hu = GetHostingUnit(ord.HostingUnitKey);
                GuestRequest gr = GetGuestRequest(ord.GuestRequestKey);
                hu.Diary = MarkingInTheDiary(hu.Diary, gr.EntryDate, gr.ReleaseDate);
                UpdateHostingUnit(hu, ord.HostingUnitKey);

                //Update the status of the guestRequest and the other Order to this guest and the other Order use this Hosting unit in this dates
                UpdateGuestRequest(ord.GuestRequestKey, Enums.RequestStatus.ClosedWithDeal);
                var orderToGuest = from order in GetAllOrders()
                                   where (order.GuestRequestKey == ord.GuestRequestKey)
                                   select order;
                orderToGuest.AsParallel().ForAll((order => UpdateOrder(order.OrderKey, Enums.OrderStatus.ClosedByHost)));
                var orderToThisDate = from order in GetAllOrders()
                                      let tgr = GetGuestRequest(order.GuestRequestKey)
                                      where (IsOverlap(gr.EntryDate, gr.ReleaseDate, tgr.EntryDate, tgr.ReleaseDate))
                                      select order;
                orderToThisDate.AsParallel().ForAll((order => UpdateOrder(order.OrderKey, Enums.OrderStatus.ClosedByHost)));
                //TODO: the boxi exception
            }
        ...
        #region IsInRange This function return if date is in the range

        /// <summary>
        /// This function return if the <paramref name="date"/> is in the range(<paramref name="startDate"/> - <paramref name="endDate"/>)
        /// </summary>
        /// <param name="startDate">Start date of the range</param>
        /// <param name="endDate">End date of the range</param>
        /// <param name="date">The date to check</param>
        /// <returns>Boolean, if the <paramref name="date"/> is in the range or not</returns>
        public bool IsInRange(DateTime startDate, DateTime endDate, DateTime date)
        {
            DateTime sd = new DateTime(2020, startDate.Month, startDate.Day);
            DateTime ed = new DateTime(2020, endDate.Month, endDate.Day);
            DateTime d = new DateTime(2020, date.Month, date.Day);
            return (d <= ed && d >= sd);
        }

        #endregion

        #region IsOverlap This function return Is there any overlap between 2 ranges

        /// <summary>
        /// This function return Is there any overlap between 2 ranges(Range1: <paramref name="entryDate1"/> - <paramref name="endDate1"/> | Range2: <paramref name="entryDate2"/> - <paramref name="endDate2"/>)
        /// </summary>
        /// <param name="entryDate1">Entry date of first range</param>
        /// <param name="endDate1">End date of first range</param>
        /// <param name="entryDate2">Start date of second range</param>
        /// <param name="endDate2">End date of seconf range</param>
        /// <returns>Boolean, if there is overlap between the range</returns>
        public bool IsOverlap(DateTime entryDate1, DateTime endDate1, DateTime entryDate2, DateTime endDate2)
        {
            DateTime entryd1 = new DateTime(2020, entryDate1.Month, entryDate1.Day);
            DateTime endd1 = new DateTime(2020, endDate1.Month, endDate1.Day);
            DateTime entryd2 = new DateTime(2020, entryDate2.Month, entryDate2.Day);
            DateTime endd2 = new DateTime(2020, endDate2.Month, endDate2.Day);
            for (DateTime dt = entryd1; dt < endDate1; dt.AddDays(1))//TODO:check if he work on all the date in the 
            {
                if (IsInRange(entryd2, endd2, dt))
                    return true;
            }
            return false;
        }
